...
class AlipaySubmit {
    ...
    function buildForm($para_temp,$gateway,$method,$button_name,$aliapy_config) {
        $para = $this->buildRequestPara($para_temp,$aliapy_config);
        $sHtml = "<form name='form' action='".$gateway."_input_charset=".trim(strtolower($aliapy_config['input_charset']))."' method='".$method."'>";
        while (list ($key,$val) = each ($para)) {
            $sHtml.= "<input type='hidden' name='".$key."' value='".$val."'/>";
        }
        $sHtml = $sHtml."<button type='submit' class='large btn primary'>绔嬪嵆璺宠浆</button>";
        return $sHtml;
    }
